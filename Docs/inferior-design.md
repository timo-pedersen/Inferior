...
- Do not lock design early. Basic stuff first.

Key difference from Elite Dangerous: ED ships are a stat sheet you buy. In Inferior, your ship is a personal investment of knowledge and tuning time. A griefer can buy the same hull, but not the 200 hours of understanding how to run it at its ceiling.
...
Full Newtonian physics with a helper layer (like ED's flight assist, but lore-grounded). No hard speed cap — the drive's standing-wave stability degrades at high offsets, giving a natural soft ceiling. "Flight assist off" as an expert mode.

### The drive — not a rocket
Ship engines create a **standing wave of ionised metal ions** partially switched through a hyperspace plane. The asymmetry this creates in normal space produces thrust. Both forward and reverse thrust are possible from the same drive. Rotational control comes from a gyroscopic effect of the drive — no separate thrusters needed.
...
Speed cap emerges naturally: at high speeds in deep space (weak gravity), alignment becomes unstable and inefficient.

### Alpha red energy
When two engines run together, they can produce **alpha red** — a special energy type that flows from plus to minus (one engine as +, the other as −). Nearly impossible to contain outside the space between the engines. Tends to leak, creating mostly harmless red streaks through air. A visible tell of a ship's power state.
...
### Mass lock
Cannot initiate hyperspace jump while a sufficiently massive object is within range. Radius = sqrt(mass) × constant. Creates genuine tension — outrun, fight, or hide. One check, large gameplay consequence.
...
Priority order: `Critical` (life support) → `High` (navigation) → `Normal` (weapons) → `Low` (luxury).

Capacitors buffer burst demand. A weapons capacitor at 80% charge fires at full rate. At 20%, fire rate drops because recharge can't keep up — fire rate is **never hard-coded**, it emerges from charge state.
...
Ship computer, lights, doors, and displays run on a **permanent battery outside the simulation entirely** — not subject to power starvation. No Half-Life flashlight syndrome.
...
Generator is **throttleable** — player sets reactor output level. Higher output = more fuel + more base heat. Creates efficient cruising vs full combat power as a real tradeoff.

### Cold start sequence
Emerges naturally from the power simulation starting from zero. No scripted sequence needed.

1. Generator spins up
2. Life support capacitor charges first
3. Navigation, then sensors
4. Weapons last
5. System alert console logs each system coming online

Player can launch before shields are fully charged. Under fire during cold start, capacitor depletes faster than it charges.
...
To go silent: throttle generator to minimum, run on capacitor reserves, cut non-essential consumers. EM drops dramatically — but capacitors drain. Hard time limit: when capacitors empty, generator must spin up and EM spikes. Tense.

Passive sensors detect EM. Active sensors detect thermal + hull reflection. Running silent against passive-only patrols works. Against active sensor sweeps, it doesn't — your hull is still there.
...
### Thermal simulation
```
Component generates heat
    │  (heat = power through × (1 - efficiency))
    ▼
Local thermal mass (component absorbs it temporarily)
    │  heat flows to coolant loop
    ▼
Coolant loop (ship-wide capacity, MW)
    │  finite flow rate
    ▼
Heat sink (hyperspace device — see below)
```

When total heat generation exceeds coolant capacity, excess stays local. When local thermal mass saturates → damage.

**Damage-efficiency feedback loop:** damage → lower efficiency → more heat → worse cooling → more damage. A spiral the player must manage by reducing load, improving cooling, or repairing.

Coolant system degrades under combat damage — hull breaches vent coolant, radiators get shot up.

### Hyperspace heat sink
Not a radiator. A **hyperspace device** that dumps heat into a separate hyperspace plane.

- Finite capacity
- Costs power to run
- When **saturated**: dumps heat back into realspace instantly — massive thermal spike and EM burst
- Makes silent running a timed resource — skilled players manage sink capacity, time their silent runs, surface for brief generator bursts, go silent again
- Pirate tactic: force a ship to vent by keeping them running silent too long
...
### Hull element system
**The ship mesh IS the damage model.** Each polygon face is a hull element made from exotic hyperspace-doped material. Hit testing is per-face. No separate damage system needed — geometry and damage are the same thing.

Hull elements:
- Come in standard **topological types** (triangle, quad, large quad) fitting across different hull geometries
- Must be produced in standard sizes — hard to post-process. This is why ships have geometric designs: **canon, not a limitation**
...
### Angular hit factor
Damage applied as cosine of incoming angle. Glancing hits deflect most energy; 90° = full energy transfer. Angled panels deflect fire naturally — **geometry IS gameplay**.

### Internal component penetration
No 3D hitboxes. Each internal component declares which hull faces protect it.

Penetration probability when a face is struck = `(1 − elementIntegrity)²`

- Pristine panel → near-zero penetration chance
- 50% damaged → meaningful chance
- Destroyed panel → near-certain penetration

Internal components have **fragility** ratings. Fuel tank = extreme (catastrophic if hit, explosion propagates). Shield generator = high. Power bus = moderate. Structural frame = low.

Ship fails when internals fail or catastrophically overheat — not when all panels are gone. A ship can theoretically fly without panels at all.

### Shields
- **Always on** — never binary. Capacitor % drives everything.
- Damage reduction = `Capacitor × MaxReduction` (MaxReduction ~0.85 — even full shields let 15% through)
- Power draw increases as capacitor depletes (shield working harder)
- Heat per hit increases at low charge → low shields = thermal trouble even if capacitor never fully depletes
...
### Philosophy
Reputation, history, and consequences follow the **player**, not the ship. Criminals cannot buy a clean ship and start fresh. Your ship is an expression of you.
...
- Equipment (shield antennas, scanners, etc. — all functional, no pure cosmetics except paint)
- Power circuit tuning (this is the main personal investment)

### Commander state
Simple sanity system — deliberately light and atmospheric. Sanity degrades on emergency jumps, recovers at stations (faster at home station).
...
**Log recovery:** The lost ship's log remains in the wreck's computer. Player can return to the wreck location and recover it — a timed mission (salvagers clear it in ~61 hours). Risk vs reward: the wreck may be in hostile space.
...
### Captain's log
- Automatic entries from game events (first system entry, combat, anomaly discovery, notable trades)
...
One static hub, multiple typed buses. Physics systems publish every frame. HUD widgets, damage system, and captain's log subscribe independently. **One publisher, many consumers — no coupling between systems.**

Sanity publishes to `DataBus.Instruments` — HUD widgets subscribe and occasionally render glitched values at low sanity. No special-casing.
...
- Cannot time-compress during jumps (multiplayer compatibility)
...
Modes may correspond to different technologies, distances, or regions of the galaxy. Inner core = flat, outer arms = tunnel networks, unknown regions = deep jumps.
...
| Item | Used by |
|------|---------|
| Hyperdrive fuel | Interstellar jumps |
| Metal rods | Engine propellant (reaction mass + energy from generator) |
| Hyperspace sink capacity | Rechargeable at port or vented |
| Ammunition | Weapons |
| Repair materials | Hull patches, component repair |
| Coolant | Lost through hull damage, topped up at stations |
...
- Internal component hitboxes: probability-based penetration from face integrity is the direction, but not fully spec'd
- Full hyperspace mode geometry for flat and tunnel types (gravity shadows, Voronoi topology)
- Generator fuel type — nuclear (no refuel needed) or consumable?
- Exact interstellar jump fuel economy
