...
- `StationGenerator.cs` — builds stations by port-to-port module attachment and collision detection. `PrepareCpu` creates module/decor geometry, flat/AO variants, megastation geometry, and procedural texture pixels without a `GraphicsDevice`; `UploadPrepared` performs texture creation and landing-pad publication on the game/render thread.
