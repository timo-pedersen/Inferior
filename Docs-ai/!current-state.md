...
1. **Lighting pipeline Phase A visual verification** — implemented, builds, tests pass
   (see "Station lighting / shadows" row above for the full write-up and the one flagged
   design/code mismatch on emissive-face detection). **Needs Timo's in-engine test pass**:
   visual parity with master at the same sun angle, correct lighting under station
   rotation (new capability), ship/containers/glass/glow sprites unchanged. Then either
   Phase B (shadow map, hull only) per the spec, or the extraction below.
2. **`StationSceneRenderer` extraction** — station mesh/glow/dot rendering out of
...
3. **`SpawnShip` vs. `ShipBuilder` convention** — `SpawnShip` still manually wires
...
4. **Debug test containers vs. `ShippingContainer`/`ShippingContainerFactory`** —
