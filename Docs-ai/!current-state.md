...
| **Authored flyable hulls / player ship cycle** | ✓ Done through Antega; propulsion visually confirmed by Timo (2026-07-20) | Aries (`type-1`), Asterisk (`asterisk`), Beren (`beren`), and Antega (`antega`) are registered semantic hulls using the same generic composite renderer and the stable cockpit `NEXT SHIP` cycle. Installed Mule, Needle, and Atlas instances now author numeric mass, forward force, maneuvering force, and future rotational torque. `ShipPropulsion` aggregates each installed operational instance through its installation orientation; `Ship.Mass` includes engine dry mass while preserving the transitional 1,200 kg default-reactor component contribution. Every Newtonian gear and atmospheric translation uses force/current mass; the legacy 900 kN ship-wide values and gear-1 fixed 4 m/s² bypass are removed. Asterisk owns provisional designed-single-engine efficiencies (0.75 forward, 0.75 maneuvering, 0.60 rotation). `ShipSnapshot.Propulsion` and the F2 debug HUD expose resolved mass, counts, force, torque, applied force, and acceleration. Configured results: Aries 78,000 kg at 4.0/2.0 m/s² forward/maneuver; Asterisk 15,600 kg at 7.5/3.75; Beren 187,800 kg at 4.0/2.0; Antega 3,585,200 kg at 4.0/1.0. Rotation remains on the old fixed-rate system; cargo mass and centre-of-mass work remain deferred. Full verification: 363 tests pass; Release build has 0 warnings and 0 errors. |
