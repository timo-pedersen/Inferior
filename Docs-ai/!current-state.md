...
| **Authored flyable hulls / player ship cycle** | ✓ Done through Beren; visually confirmed by Timo (2026-07-19) | Aries (`type-1`), Asterisk (`asterisk`), and Beren (`beren`) are registered semantic hulls using the same generic composite renderer. Asterisk is the accepted 8.6 m one-container hauler with one port Mule and a starboard C2 command blister. Beren is the accepted medium 27 m by 20 m cargo platform: 3-by-3 canonical-container design volume, closed visual aft cargo door, four independently installed Needle engines in vertical port/starboard pairs, and a complete forward underslung C2 cockpit looking 10° down. The generic camera-originated ship-forward reticle therefore appears left of centre on Asterisk and above centre on Beren; own-ship geometry remains hidden in first-person. The cockpit CTRL rail's `NEXT SHIP` button requests the stable Aries → Asterisk → Beren → Aries cycle. `SpaceSimulation` performs replacement on the simulation thread and preserves position, velocity, orientation, and flight state; `SystemSpaceState` no longer retains a stale mutable ship reference across map transitions. The mount-count-agnostic default engine installer handles one, two, or four engines independently; pair-only debug replacement declines ambiguous multi-engine layouts. Full verification: 263 tests pass; Release build has 0 warnings and 0 errors. |
